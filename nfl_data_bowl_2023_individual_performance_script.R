...
# Note that these datasets can be accessed through the original competition website:
...
week2 <- read.csv("~/data/week2.csv")
week3 <- read.csv("~/data/week3.csv")
week4 <- read.csv("~/data/week4.csv")
week5 <- read.csv("~/data/week5.csv")
week6 <- read.csv("~/data/week6.csv")
week7 <- read.csv("~/data/week7.csv")
week8 <- read.csv("~/data/week8.csv")
...
ggsave(filename = "~/defensive_ends.png", plot = DE.p,
...
ggsave(filename = "~/guards.png", plot = G.p,
       height=8,width=12, units="in")
...
sl.p <- ggplot(final_data_sl, aes(true_time,max_acc)) +
  ...
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"),
        plot.title= element_text(size=18, face="bold")) +
  geom_point(pch=21, colour="black",fill="red2", size=5) +
...
ggsave(filename = "~/shaw_lawson.png", plot = sl.p,
       height=8,width=12, units="in")
